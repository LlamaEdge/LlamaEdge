...
pub async fn chat_prompt_template(
    name: Option<&str>,
    ...
    match name {
        Some(name) => match chat_graphs.get(name) {
            Some((_, graph)) => {
                let prompt_template = { graph.lock().await.prompt_template() };
...
pub(crate) fn get_output_buffer_single(
    ...
    #[cfg(feature = "logging")]
    debug!(target: "llama-core", "Get output buffer generated by the model named {} in the stream mode.", graph.name());
...
pub(crate) async fn get_token_info_by_graph_name(
    name: Option<&String>,
